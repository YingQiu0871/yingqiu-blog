...
| ----------- | ---- | ---------------- | ------------------------ |
| `fuguang`   | 浮光 | `/zh/fuguang/`   | 日常、旅行、心情与当下   |
| `jiujian`   | 旧笺 | `/zh/jiujian/`   | 童年、小学到高中时期的旧文 |
| `liusheng`  | 流声 | `/zh/liusheng/`  | 音乐、歌单与歌的记忆     |
| `qiushu`    | 求索 | `/zh/qiushu/`    | 求学、科研与实验室       |
| `shiju`     | 拾句 | `/zh/shiju/`     | 摘抄、诗词与阅读片段     |
...
旧笺按 `originalDate` 升序排列（童年在前），其余栏目按 `date` 倒序。草稿不会出现在列表、RSS 和站点地图里，但可以用 `npm run dev` 在本地预览。
